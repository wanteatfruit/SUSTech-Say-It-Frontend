...
export function addViewCount(title:string) {
    axios.post(host+`/post/addviewcount?title=${title}`).then(resp=>{
...
export const addUpVote = (title:string) =>{
    ...
    axios_instance.post(host+`/post/addupvote?title=${title}`).then(resp=>{
...
export const addDownVote = (title:string) =>{
    var data;
    axios_instance.post(host+`/post/adddownvote?title=${title}`).then(resp=>{
        console.log(resp.data)
        data = resp.data;
        return data;
    }).catch((e)=>{
...
export async function getPostsBySpace(spaceID:number) {
    const res = await fetch(host+`/post/getallpostbyspaceid?spaceid=${spaceID.toString()}`);
    const data = await res.json()
...
export async function getCommentsBypostID(postID:string): Promise<commentInterface[]>{
    const res = await fetch(host+`/comment/getallcommentbypostid?postid=${postID}`);
    const data = await res.json()
    return data;
}
...
export async function getPostStatus(post_id:string){
    try{
        ...
        const res = await axios_instance.get(host+`/post/getPostStatus?post_id=${post_id}`);
...
export async function deleteUpvote(title:string){
    const res = await axios_instance.post(host+`/post/deleteupvote?title=${title}`);
    const data = await res.data
    return data;
}

export async function deleteDownvote(title:string){
    const res = await axios_instance.post(host+`/post/deletedownvote?title=${title}`);
    const data = await res.data
    return data;
}